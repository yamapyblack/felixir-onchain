...
describe("FelixirExecutor-test", async () => {
  ...
  describe("test", async () => {
    it("fail onlyAdmin", async () => {
      await expect(c.connect(addr1).setLogic(addr1.address)).revertedWith(
        "FelixirExecutor::queueTransaction: Call must come from admin."
      );
      await c.setAdmin(NilAddress);
      await expect(c.setLogic(addr1.address)).revertedWith(
        "FelixirExecutor::queueTransaction: Call must come from admin."
      );
    });
    ...
    it("fail onlyLogic", async () => {
      ...
      await expect(
        ...
        "FelixirExecutor::queueTransaction: Call must come from logic contract."
      );
      await expect(
        c
          .connect(addr1)
    ...
    it("fail executeTransaction No1", async () => {
      ...
      await expect(
        ...
        "FelixirExecutor::executeTransaction: Transaction hasn't been queued."
      );
    });
    ...
    it("fail executeTransaction No4", async () => {
      ...
      await expect(
        ...
        "FelixirExecutor::executeTransaction: Transaction execution reverted."
      );
    });
